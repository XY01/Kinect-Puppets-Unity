...
{	
	...
	public static float Scale( this float val, float fromMin, float fromMax, float toMin, float toMax, bool clamp )
	{		
		float fromRange = 	fromMax - fromMin;
		float toRange = 	toMax - toMin;
	...
	public static float ScaleTo01( this float val, float min, float max, bool clamp )
	{		
		float range = max - min;
		if (range == 0)
		{
	...
	public static float ScaleFrom01( this float normaliedVal, float min, float max, bool clamp )
	{		
		float val;
		if( min > max )
		{
		...
		val = min + ( (max - min) * normaliedVal );
		if( clamp )	val = Mathf.Clamp( val, min, max );
